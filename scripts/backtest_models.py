...
from football_forecast_lab.ledger import load_jsonl
...
def main() -> None:
    ...
    ledger_result = evaluate_ledger_predictions(rows)
    ml_result = json.loads(args.ml_result.read_text(encoding="utf-8")) if args.ml_result.exists() else {}
    args.output.parent.mkdir(parents=True, exist_ok=True)
    args.output.write_text(build_report(ledger_result, ml_result, args.ledger), encoding="utf-8")
    print(f"OK: wrote {args.output}")


def build_report(ledger_result: dict, ml_result: dict, ledger_path: Path) -> str:
    lines = [
        ...
        f"Rows: `{ledger_result.get('rows', 0)}`; resolved: `{ledger_result.get('resolved', 0)}`; pending: `{ledger_result.get('pending', 0)}`.",
        "",
    ]
